...
| Список данных (сделки, сетапы, идеи) | Type C (data-list) | L2 метрика, L1 строка (list) / L2 Data card (grid 3 col) | §3 |
...
- **Mobile (`< md`):** 2-рядная компактная строка `min-h-[4.25rem] rounded-[18px] px-3.5 py-3`: первый ряд — `Symbol + Session + Status` слева и `Primary metric` справа; второй ряд — `Date + Tags/Notes` слева и `Risk` справа. Thumbnail справа с hit-target `min-w-11 min-h-11`[cite: 7].

#### 2. ViewMode: Grid (3-column Data Cards)
- **Сетка:** строго 3 колонки на десктопе: `grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4`[cite: 7].
  - Mobile (`< 640px`): 1 колонка.
  - Tablet (`sm` / `md`): 2 колонки.
  - Desktop (`lg` и выше): 3 колонки.
- **Surface:** полноценный L2-контейнер `bg-card border border-border-card rounded-[26px] p-3.5 sm:p-4 shadow-xs` (в отличие от Dashboard Widgets §3 Type A, карточки данных имеют физическую рамку `border-border-card` и представляют отдельные записи данных)[cite: 7].
- **Компактность:** никаких раздутых высот и искусственных `min-h-[190px]` — естественная флекс-структура `flex flex-col justify-between h-full`.

#### 3. Анатомия Data Card (Grid)
Карточка данных строится по строгой 3-зонной анатомии (Header ⇄ Body ⇄ Footer):
1. **Header (Identity & Primary Metric):**
   - Слева: направление/тип (L0 icon-badge `w-8 h-8 rounded-[14px]`, иконка 15px), символ/тикер (`text-sm font-bold tracking-tight text-text-main`), сессионный тег (`px-1.5 py-0.5 rounded-[10px] bg-canvas border border-border-card text-[0.6875rem] font-mono uppercase`), статусный бейдж (`rounded-full px-2 py-0.5 text-[0.6875rem] font-bold uppercase`).
   - Справа: главная метрика (напр. PnL `text-sm font-bold font-mono tabular-nums` + вторичное процентное изменение `text-[0.6875rem] font-mono tabular-nums`).
   - Разделитель: `pb-2.5 border-b border-border-card/60`.
2. **Body (Tags, Notes, Media preview):**
   - Верх: чипы сетапа/ошибок (L0 `rounded-[10px] px-1.5 py-0.5 text-[0.6875rem] font-medium`).
   - Центр: краткий текст заметки `text-xs text-text-muted line-clamp-2 leading-relaxed font-normal` (без искусственных min-height при пустых заметках).
   - Справа: компактный медиа-thumbnail `w-12 h-9 sm:w-14 sm:h-10 rounded-[14px] overflow-hidden border border-border-card bg-canvas` со счетчиком `+N`.
   - Ритм: `py-2.5 flex-1 flex flex-col justify-between gap-2`.
3. **Footer (Parameters & Timestamp):**
   - Слева: ключевой параметр (напр. `Risk <strong className="font-mono tabular-nums font-semibold text-text-main">X%</strong>`).
   - **Правило исключения RR 1:...:** Значения соотношения риск/прибыль в формате `RR 1:...` категорически не выводятся на карточках данных (ни в Grid, ни в List) — результат сделки уже отражён в R в Header, дублирование перегружает карточку[cite: 7].
   - Справа: дата события (`font-mono tabular-nums text-[0.6875rem] text-text-muted`).
   - Разделитель: `pt-2 border-t border-border-card/60`.

#### 4. Пагинация и Load More
- **Размер порции:** 12 элементов (кратно 3 колонкам: 4 ряда по 3).
- **Кнопка "Load more":** отображается при `totalCount > visibleCount` по центру под сеткой/списком. Реализуется как стандартная L1 secondary pill-кнопка (§4 Button): `h-10 min-h-11 md:min-h-10 px-5 rounded-full bg-card border border-border-card text-text-main hover:bg-canvas active:scale-[0.98] text-xs font-semibold inline-flex items-center gap-2 shadow-xs`. Включает лейбл `Load more`, счетчик оставшихся элементов `(N)` и иконку `ChevronDown` 14px.
- **Анимация подгрузки (Stagger):** новые карточки появляются плавно каскадом:
  - `initial={{ opacity: 0, y: 10, scale: 0.98 }}`, `animate={{ opacity: 1, y: 0, scale: 1 }}`.
  - Задержка: `delay = Math.min((index % 12) * 0.035, 0.24)s`, duration `0.2s`, ease `[0.16, 1, 0.3, 1]`.
  - Предыдущие элементы сохраняют положение; анимируется только новая порция.
- **Сброс:** смена вкладки, поискового запроса, любого фильтра или направления сортировки мгновенно сбрасывает пагинацию к начальным 12 элементам.
...
**Parameters:** `bg-card border border-border-card rounded-[26px] p-5`; компакт-вариант `p-3.5 sm:p-4`[cite: 7]. Для карточек данных в списках Type C (Data Cards в 3-колоночном Grid) действует строгий стандарт компактности и 3-зонной анатомии (§3 Type C), без раздутых высот и избыточных дублирующих параметров (таких как `RR 1:...`)[cite: 7].
...
**1. Геометрия.** Header — нативная полупрозрачная область матового размытия (iOS frosted glass) без резких границ, швов и линий: `h-16 sticky top-0 z-20 px-4 bg-canvas/80 backdrop-blur-xl shrink-0`, **без `border-b`**, с учётом safe-area сверху (`pt-[env(safe-area-inset-top)]`). Хедер расположен **внутри единого скролл-контейнера модалки**: в состоянии покоя (при скролле вверху) его фон полностью и бесшовно сливается с фоном холста экрана (`bg-canvas`), не образуя отдельной прямоугольной «карточки» или контрастной плашки; при скролле контент формы физически заезжает под хедер и мягко размывается под кнопками управления и заголовком. Body — `px-6 py-5`, с обязательным нижним safe-area padding (`pb-[calc(env(safe-area-inset-bottom)+24px)]`).
...
### Единый канонический Shell (ModalShell / BaseModal)

**Архитектурное правило:** В приложении строго запрещено создавать самописную разметку модальных окон (ручные flex/fixed контейнеры, дублирующие оверлеи). Все модальные диалоги системы (формы трейдов, торговых идей, сетапов плейбука, настройки счетов, редактирование разделов, авторизация, чат ассистента и диалоги подтверждения) обязаны строиться исключительно на базе единого канонического компонента `ModalShell` (или высокоуровневого `BaseModal`, реализованного поверх него).

**Что обеспечивает единый ModalShell:**
1. **Единый scroll container и frosted-хедер:** Мобильный матовый хедер (`bg-canvas/80 backdrop-blur-xl shrink-0`, строго без `border-b`) всегда расположен **внутри** единого скролл-контейнера `overflow-y-auto`. Контент формы/чата при скролле физически заходит под него и размывается; в исходном положении (scroll=0) хедер на 100% бесшовно сливается с фоном `bg-canvas`. Категорически запрещено выносить мобильный хедер за пределы скролл-контейнера или задавать телу модалки контрастные фоны (`bg-canvas/30`, `bg-card`), дробящие экран на карточки.
2. **Безопасные зоны (Safe Areas):** Автоматический учёт верхнего отступа хедера `pt-[env(safe-area-inset-top)]` и нижнего отступа `pb-[calc(env(safe-area-inset-bottom)+24px)]`.
3. **Единый жизненный цикл оверлея:** Автоматическое подключение `lockBodyScroll()` при открытии и освобождение при закрытии; единая обработка клавиши `Escape`; единая Framer Motion анимация (`y: 8→0, opacity 0→1, duration: 0.18s, layout spring`).
4. **Стандартизированные L1-контролы:** Кнопки хедера на мобилке строго `w-11 h-11` (hit target 44×44px), на десктопе — `w-10 h-10` с физической подложкой `bg-canvas border border-border-card`.
...
14. Списки данных Type C (сделки, идеи, сетапы): в режиме Grid строятся строго в 3 колонки (desktop) с компактным паддингом `p-3.5 sm:p-4` и строгой 3-зонной анатомией (Header / Body / Footer); параметры `RR 1:...` не отображаются; при >12 элементах обязательна кнопка Load More со stagger-анимацией новой партии.