...
export function BestSessionWidget({ size, stats }: WidgetProps) {
      ...
      <WidgetCard size={size} className="justify-between text-left">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[0.6875rem] font-bold tabular-nums px-1.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-500">
            {bestSession.winRate.toFixed(0)}% WR
          </span>
        </div>
        ...
        <div className="w-full">
          <div className="flex items-center justify-between text-[0.6875rem] text-text-muted mb-1 font-mono">
            ...
            <span>{bestSession.winTrades}W</span>
          </div>
          <div className="w-full bg-canvas rounded-full h-1.5 overflow-hidden">
            <div 
              className="bg-emerald-500 h-full rounded-full transition-all duration-300" 
              style={{ width: `${Math.min(100, Math.max(0, bestSession.winRate))}%` }} 
            />
          </div>
        </div>
      </WidgetCard>
  ...
  // Medium size
  return (
    <WidgetCard title="Top Trading Session" size={size}>
      {!bestSession ? (
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          <Trophy size={20} className="text-emerald-500/40 mb-1" />
          <div className="text-sm font-semibold text-text-main">No Session Edge</div>
          <div className="text-xs text-text-muted mt-0.5">Log trades with session tags to view edge</div>
        </div>
      ) : (
        <div className="flex items-center justify-between gap-4 flex-1 h-full">
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-xs text-text-muted font-mono">
                ({bestSession.tradesCount} trades)
              </span>
            </div>
          </div>
          ...
          <div className="w-48 bg-canvas rounded-[14px] p-3 flex flex-col justify-between shrink-0 h-full">
            ...
            <div className="flex items-center justify-between text-[0.6875rem] text-text-muted font-mono">
              <span>{bestSession.winTrades} Wins</span>
              <span>{bestSession.lossTrades} Losses</span>
            </div>
          </div>
        </div>
      )}
